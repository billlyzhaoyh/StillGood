...
def load_model_server():
	global model
	model_name = 'tf_serving_keras_resnetbm.h5'
...
def predict():
	...
	if flask.request.method == "POST":
		if flask.request.files.get("image"):
			...
			#now we are predicting one of the six categories
			...
			item=labels[int(predicted_class)]
			data["predictions"]=ingredients[item]
